...
module.exports = {
    GetReviewData:(review_id,callback) => {
        pool.query(
            `SELECT *,ka_collect_url.is_pushed_cloud,ka_industry_cat.name as industry_cat_name ,ka_sub_cat.name as industry_sub_cat_name FROM ka_collect_url LEFT JOIN ka_industry_cat ON ka_collect_url.industry_cat = ka_industry_cat.id LEFT JOIN ka_sub_cat ON ka_collect_url.industry_sub_cat = ka_sub_cat.id WHERE ka_collect_url.unique_uid = ? AND ka_collect_url.is_used = 0`,
            [
                review_id
            ],
